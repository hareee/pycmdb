...
class AnsibleTask:
    def __init__(self, hosts, extra_vars=None):
        ...
        self.options = Options(connection='paramiko', module_path=None, forks=10,
    ...
    def exec_shell(self, command):
        source = {'hosts': 'all', 'gather_facts': 'no', 'tasks': [
            {'action': {'module': 'command', 'args': command}, 'register': 'shell_out'}]}
...
if __name__ == "__main__":
    task = AnsibleTask([AnsibleHost('134.64.14.41', 22, 'paramiko', 'root', 'Hello@163')])
    #task.exec_playbook(['/install.yml', 'init.yml'])
    print(task.exec_shell('df -h'))